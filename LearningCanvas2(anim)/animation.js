/*attempting placing distance func in the Cir class*/
...
function Circle(x, y, dx, dy, radius, color, otherCir){
    this.x=x;
    this.y=y
    this.dx=dx;
    this.dy=dy;
    ...
    this.update=function(){  //putting distance func here      

        if (this.x+this.radius>innerWidth|| this.x-this.radius<0){this.dx=-this.dx}
        if (this.y+this.radius>innerHeight|| this.y-this.radius<0){this.dy=-this.dy}

        if (distance(this.x, this.y, this.otherCir.x, this.otherCir.y)<this.radius+this.otherCir.radius)
...
var circle1=new Circle(200,200, 5, 3, 50, "blue", circle2);
