...
export const button=document.getElementById("mysubmit");
...
button.addEventListener('click', function(){
    ans =document.getElementById("input1").value;
    if (ans==num1+num2){response.innerHTML="that is the correct answer";  }      
    
    else{response.innerHTML="try adding on paper";
        ansList.push(ans)
        if (ansList.length>3){
            let probList=ansList;
            response.innerHTML=`these are your troubling answers: ${probList}`;
            button.innerHTML="stop and play";
            }
        }
